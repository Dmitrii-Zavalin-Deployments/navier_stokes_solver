...
def test_solver_heartbeat_logging(tmp_path, caplog):
    ...
    Fixes:
    - F841: Removed unused patch assignments.
    - TypeError: Provides real floats for state.time comparison.
    - ValueError: Returns (None, delta) tuple for Step 3 unpacking.
    """

    # 1. Setup real dummy objects to satisfy schema and comparisons
    input_file = tmp_path / "input.json"
    dummy_input = create_validated_input(nx=2, ny=2, nz=2)
    # Ensure total_time is set to a float to prevent TypeError at line 128
    dummy_input.simulation_parameters.total_time = 1.0 
    input_file.write_text(json.dumps(dummy_input.to_dict()))

    # 2. Patch the pipeline. mock_load_call used to avoid F841.
    with patch("src.main_solver._load_simulation_context") as mock_load_call, \
        ...
        # Setup Context with the REAL dummy_input object
        ...
        # Setup State (time must be a float for >= comparison)
        ...
        # Fix ValueError: return (None, delta) for unpacking in the PPE loop
        mock_step3.return_value = (None, 0.0)

        # Configure Elasticity for log formatting check
