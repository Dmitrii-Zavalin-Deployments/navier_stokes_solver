...
def fill_ghost_boundaries(state: SolverState) -> None:
    ...
    """
    # Fix the NameError by extracting dimensions early
    ...
    def apply_face_bc(face_name, axis, side):
        ...
        # Expanded Logic to handle Y and Z axes for Pressure
        if bc_type == "pressure":
            val = config["p"]
            if axis == 0: # X
            ...
            # Default to Zero-Gradient (Neumann) for all axes
            if axis == 0:
    ...
    if DEBUG:
        # Check X-axis signals as a proxy for sync health
