...
def get_dt_over_rho(block: StencilBlock) -> float:
    ...
    if not np.isfinite(scaling):
        logger.error(f"NUMERICAL INSTABILITY: dt/rho exploded in {block.id} (dt={block.dt}, rho={block.rho})")
...
def get_rho_over_dt(block: StencilBlock) -> float:
    ...
    # Forensic Audit
    if not np.isfinite(scaling):
        logger.error(f"NUMERICAL INSTABILITY: rho/dt exploded in {block.id} (rho={block.rho}, dt={block.dt})")
