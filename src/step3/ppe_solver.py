...
def solve_pressure_poisson_step(
    ...
    # Catch non-finite divergence values immediately to avoid poisoning the field
    ...
    # Access Neighbors via FI.P_NEXT to support in-place SOR convergence acceleration
    sum_neighbors = (
    ...
    # Explicit audit using injected threshold parameter
    p_old_val = np.atleast_1d(p_old)
    if not np.isfinite(p_old_val).all() or np.any(np.abs(p_old_val) > divergence_threshold):
        logger.error(f"PPE CRITICAL: Poisoned p_old in block {block.id} | Limit: {divergence_threshold:.1e}")
        raise ArithmeticError(f"Poisoned Pressure Trial: {divergence_threshold}")
    ...
    # Rule 0: Robust delta calculation using numpy max identity
    delta = float(np.max(np.abs(p_new - p_old), initial=0.0))
    
    # 6. Direct write-back to foundation
